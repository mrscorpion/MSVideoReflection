...
//#import "LeafNotification.h"  // 顶部通知提示框
#import "AudioViewController.h" 
//#import "MusicListController.h" // 音乐列表
#import <MediaPlayer/MediaPlayer.h>

#define MaxVideoLength 30 //10
...
ScrollSelectViewDelegate
>
{
    CMPopTipView *_popTipView;
//    LeafNotification *_notification; // 联系我们的顶部通知
}
...
- (void)showCustomActionSheet:(UIBarButtonItem *)barButtonItem withEvent:(UIEvent *)event
{
    ...
    JGActionSheetSection *sectionEmbededObjects = [JGActionSheetSection sectionWithTitle:embededObjectsTitle message:nil buttonTitles:@[GBLocalizedString(@"Gif"),GBLocalizedString(@"Video"),GBLocalizedString(@"BackgroundMusic")] buttonStyle:JGActionSheetButtonStyleDefault];
    ...
    // 开始创建StartToCreate
    JGActionSheetSection *sectionResult = [JGActionSheetSection sectionWithTitle:resultTitle message:nil buttonTitles:@[ GBLocalizedString(@"StartToCreate")] buttonStyle:JGActionSheetButtonStyleDefault];
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
    ...
    if([mediaType isEqualToString:@"public.movie"])
    {
        ...
        // 抱歉, 视频文件长度目前暂定为小于:MaxVideoLength (视频长度选择控制小于  < MaxVideoLength)
...
//            NSString *file = [item objectForKey:@"url"];
            ...
            NSArray *allAudios = queue.items;
            MPMediaItem *mediaItem =  allAudios[index];
            NSURL *assetURL = mediaItem.assetURL;
            NSLog(@"assetURL-->%@, path--%@", assetURL, assetURL.path);
//            NSString *file = [assetURL relativePath];
//            NSString *file = [NSString stringWithFormat:@"%@", assetURL];
//            [[SRScreenRecorder sharedInstance] setAudioOutPath:file];
            [[SRScreenRecorder sharedInstance] setAudioOutPath:assetURL];
...
//            NSString *audioReadyHint = [NSString stringWithFormat:@"%@%@(%@)", GBLocalizedString(@"AudioContent"), GBLocalizedString(@"Ready"), [item objectForKey:@"song"]];
            NSString *audioReadyHint = [NSString stringWithFormat:@"%@%@(%@)", GBLocalizedString(@"AudioContent"), GBLocalizedString(@"Ready"), mediaItem.title];
...
- (void)viewDidLoad
{
    ...
    self.view.backgroundColor = [UIColor lightGrayColor]; //[UIColor colorWithPatternImage:[UIImage imageNamed:@"sharebg3"]];
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    // Hide scroll view 隐藏底部的gif和边框scrollview
...
#pragma mark - 显示和隐藏底部的GIF和边框collectionView
...
- (void)screenRecording
{
    ...
    [[SRScreenRecorder sharedInstance] setFinishRecordingBlock: ^(BOOL success, id result) {
        dispatch_async(dispatch_get_main_queue(), ^{
            if (success) {
                // 新增：保存成功隐藏掉底部的scrollview（GIF和边框）
                [self hiddenBottomControlView];
